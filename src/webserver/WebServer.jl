...
"""
function run(host, port::Union{Missing, Integer}; launchbrowser::Bool=true, session=ServerSession())
    ...
    if launchbrowser
        if Sys.iswindows()
            Base.run(`cmd.exe /c "start $address"`)
        elseif Sys.islinux()
            Base.run(`bash -c "start $address"`)
        end
    end
